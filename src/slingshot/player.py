...
#		self.image = pygame.transform.rotate(image1, -self.rel_rot)
    ...
    def get_launchpoint(self):
        if Settings.ROTATE:
            return (self.rect.center[0] + self.d
                    * math.sin(math.radians(self.angle)),
                    self.rect.center[1] - self.d
    ...
    def draw_info(self, screen):
        txt = Settings.font.render("Angle: %3.2f" %
                                   (self.angle), 1, (255, 255, 255))
        rect = txt.get_rect()
        rect.topleft = (290, 5)
        screen.blit(txt, rect.topleft)

        txt = Settings.font.render("Power: %3.1f" %
                                   (self.power), 1, (255, 255, 255))
        rect = txt.get_rect()
        rect.topleft = (403, 5)
        screen.blit(txt, rect.topleft)
    ...
    def draw_status(self, screen):
        if self.player == 1:
            txt = Settings.font.render(
                "{}  --  {}".format(self.get_name(), self.score), 1, self.color)
            rect = txt.get_rect()
            rect.topleft = (5, 5)
        else:
            txt = Settings.font.render(
                "{}  --  {}".format(self.score, self.get_name()), 1, self.color)
            rect = txt.get_rect()
            rect.topright = (794, 5)
        screen.blit(txt, rect.topleft)
