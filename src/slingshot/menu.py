...
class Menu:
    ...
    def change_active(self, item, a):
        for i in xrange(0, self.items.__len__()):
            if self.items[i][0] == item:
                self.items[i] = (self.items[i][0], self.items[i]
                                 [1], self.items[i][2], a)
    ...
    def select(self):
        if self.items[self.selected][1]:
            self.items[self.selected] = (self.items[self.selected][0],
    ...
    def draw(self):
        ...
        for i in xrange(0, n):
            ...
            if self.items[i][1] == 1:
                offset = 35
                result.blit(
                    Settings.box, (25, 2.5 * Settings.MENU_LINEFEED
                                   + Settings.MENU_LINEFEED * i - 8))
                if self.items[i][2]:
                    if self.items[i][3]:
                        result.blit(
                            Settings.tick, (25, 2.5 * Settings.MENU_LINEFEED
                                            + Settings.MENU_LINEFEED * i - 8))
                    else:
                        result.blit(Settings.tick_inactive,
                                    (25, 2.5 * Settings.MENU_LINEFEED
                                     + Settings.MENU_LINEFEED * i - 8))
            else:
                offset = 0
            result.blit(txt, (25 + offset,
                              2.5 * Settings.MENU_LINEFEED
...
class Numeric(Menu):
    ...
    def draw(self):
        ...
        if self.val == 0:
            ...
            txt = Settings.menu_font.render(
                "%d" % (self.val), 1, (255, 255, 255))
        rect = txt.get_rect()
