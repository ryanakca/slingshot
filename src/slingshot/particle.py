...
class Particle(pygame.sprite.Sprite):
    ...
    def __init__(self, pos=(0.0, 0.0), size=10):
        ...
        angle = randint(0, 359)
        if size == 5:
            speed = randint(Settings.PARTICLE_5_MINSPEED,
                            Settings.PARTICLE_5_MAXSPEED)
        else:
            speed = randint(Settings.PARTICLE_10_MINSPEED,
                            Settings.PARTICLE_10_MAXSPEED)
    ...
    def update(self, planets):
        ...
        for p in planets:
            ...
            #       [      r ^ (1/3)                 r ^ (1/3)        ]
            try:
                a = ((Settings.g * mass * dx) / (d * math.sqrt(d)),
                     (Settings.g * mass * dy) / (d * math.sqrt(d)))
        ...
        for p in planets:
            ...
            if p.type == "Blackhole":
                ...
                # This is a planet
                self.impact_pos = get_intersect(
                    p_pos, r, self.last_pos, self.pos)
                self.pos = self.impact_pos
                return 0
        ...
        if Settings.BOUNCE:
            ...
            if self.pos[1] > 599:
                ...
                self.pos = (self.last_pos[0] + (self.pos[0] - self.last_pos[0])
                            * (599 - self.last_pos[1]) / d,
                            599)
                self.v = (self.v[0], -self.v[1])
            if self.pos[1] < 0:
                d = self.last_pos[1] - self.pos[1]
                self.pos = (self.last_pos[0]
...
class Missile(Particle):
    ...
    def update_players(self, players):
        ...
        for i in xrange(10):
            pos = (self.last_pos[0] + i * 0.1 * self.v[0],
    ...
    def draw_status(self, screen):
        txt = Settings.font.render("Power penalty: %d" %
    ...
    def update(self, planets, players):
        ...
        if result != -1:
            pygame.draw.aaline(self.trail_screen,
                               self.trail_color, self.last_pos, self.pos)
        return result
